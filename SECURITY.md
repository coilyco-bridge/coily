...
- **Package-manager passthroughs do not launder arbitrary-code execution.** Bare interpreters and scratch-tier execution are denied at two layers: the lockdown deny-list blocks bare `python` / `bash` / etc., and cli-guard's PreToolUse engine blocks both interpreter tokens and direct execution of a file under a writable scratch tier (`/tmp`, `/var/tmp`, `/dev/shm`, `/private/tmp`, `/private/var/tmp`) - so `chmod +x /tmp/foo && /tmp/foo` is refused at exec time even though no interpreter token appears (cli-guard#87, closes coily#9). That engine deny classifies the *leading token* of a Bash segment, so it cannot see a path buried in `coily pkg uv run /tmp/x.py` (the segment leads with `coily`). The matching coily-side check is the `pkg uv` preflight gate (`uvPreflightGate`): `uv run` / `uv tool run` are refused when a script-path argument sits in a scratch tier or escapes the repo root, since `uv run PATH` is a general-purpose Python interpreter (closes coily#10). Project-rooted script paths and bare tool/module names (`uv run pytest`, `uv run ruff check`) pass; the non-`run` uv verbs coily itself relies on (`uv tool install`, `uv pip`, `uv sync`) are never gated. Runtime verification: `TestUvRunPathViolation_Blocked` / `_Allowed` and `TestUvPreflightGate_BlocksScratchRun` in `cmd/coily/uv_run_gate_test.go`.
