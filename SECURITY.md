...
- Tailscale, GitHub, and whatever else shells out through `gh`/`git`
...
- **Deny broadly.** Every execution-family CLI. The scripting interpreters (python, ruby, perl, deno, node), `go run`, the shells (sh, bash, zsh, plus Windows cmd / powershell / pwsh), Windows scripting hosts and LOLBAS binaries (wscript, cscript, mshta, rundll32, regsvr32), build runners (`make`, package manager scripts), and `kubectl`, `helm`, `terraform`, `aws`, `gcloud`, `gh`, `docker` wholesale. `ssh` / `scp` are denied wholesale (coily no longer has any SSH surface). The non-Bash `PowerShell` tool that Claude Code exposes on Windows is denied wholesale, the same way MCP tools are, since it bypasses the Bash prefix matcher entirely.
...
- **Sub-tool binaries (`aws`, `kubectl`, `gh`, `tailscale`, etc.) are resolved via `$PATH`.** An earlier version of coily pinned each by sha256 in an embedded manifest, fetched them on first use from a GitHub Release, and bypassed `$PATH` entirely. That machinery was removed: the threat it addressed (an attacker with write to a `$PATH` directory but not to `$HOME`, where the cache and the coily binary itself live) was a narrow slice that did not justify the release pipeline + manifest + per-tool refresh cadence. The actual safety boundary is argv validation (no shell metacharacters reach the subprocess), the audit log (every invocation is recorded), and the lockdown deny list (raw `aws` / `kubectl` / `gh` are denied at the Bash-tool layer so the agent can only reach them via `coily`). Binary authenticity below that is the host's problem - the same problem `brew` and `apt` and every other package manager have. Coily no longer reaches kai-server over SSH at all: the `cli-guard/ssh` transport and the `coily ssh` passthrough were removed, and the kai-server ops verbs (systemctl / eco / factorio / forgejo) now run only on kai-server itself. Bare `ssh` / `scp` stay denied at the Bash-tool layer with no coily wrapper to reach them.
...
- **No `coily shell` / `coily run` escape hatch**, ever. The moment one exists, the whole boundary collapses. There is no remote-shell surface either: the free-form `coily ssh <alias> -- <coily args>` passthrough (once tracked in [coily#187](https://github.com/coilysiren/coily/issues/187)) and the underlying `cli-guard/ssh` transport were both removed. The kai-server ops verbs run locally on kai-server, and bare `ssh` / `scp` are denied with no wrapper to reach them. There is no `coily ops kubectl exec` pass-through either: the lockdown deny-list catches the exec form before coily sees it.
- **Package-manager passthroughs do not launder arbitrary-code execution.** Bare interpreters and scratch-tier execution are denied at two layers: the lockdown deny-list blocks bare `python` / `bash` / etc., and cli-guard's PreToolUse engine blocks both interpreter tokens and direct execution of a file under a writable scratch tier (`/tmp`, `/var/tmp`, `/dev/shm`, `/private/tmp`, `/private/var/tmp`) - so `chmod +x /tmp/foo && /tmp/foo` is refused at exec time even though no interpreter token appears (cli-guard#87, closes coily#9). That engine deny classifies the *leading token* of a Bash segment, so it cannot see a path buried in `coily pkg uv run /tmp/x.py` (the segment leads with `coily`). The matching coily-side check is the `pkg` preflight gate (`pkgRunGate`, registry in `cmd/coily/pkg_run_gate.go`): a tool's run/exec verb is refused when a script-path argument sits in a scratch tier or escapes the repo root, since those verbs are general-purpose interpreters. Gated tools and their run/exec shapes: `uv run` / `uv tool run`, `bun` (bare-file exec + `run`), `pipx run`, `poetry run`, `bundle exec`, `npm exec`/`x`, `pnpm`/`yarn` `exec`/`dlx`, and `nix` `run`/`eval`/`shell`/`develop` (closes coily#10, and the laundering half of coily#9). Project-rooted script paths and bare tool/module names (`uv run pytest`, `bun run build`, `pnpm exec eslint src/`) pass; non-run verbs coily relies on (`uv tool install`, `npm install`, `nix build`) are never gated. `pip` / `gem` (install-time setup-code execution) and `cargo` (repo-defined run targets) are intentionally not registered - different threat classes. Runtime verification: `TestPkgRunPathViolation_Blocked` / `_Allowed`, `TestPkgRunGate_WiredBehavior`, and `TestPkgRunSpecs_MatchPtPkgWiring` in `cmd/coily/pkg_run_gate_test.go`.
- **Read-only `coily ops aws` invocations are denied pre-send when they touch a sensitive resource pattern.** A read-only aws verb (`s3 ls`, `ec2 describe-*`, `iam list-*`, `sts get-caller-identity`, and the rest of the describe/list/get/head/lookup/search/select/batch-get family) used to land an audit row and pass straight through with no argv validation. The audit row documents a leak; it is not the layer that prevents the read. Read-only invocations still exfiltrate (`s3 ls` on a secrets bucket), confirm threat-model state, and enumerate, and CloudTrail is post-hoc while wide iam scoping is lazy reuse, so neither stops the read happening in the first place. The `awsReadGate` preflight gate (`cmd/coily/aws_read_gate.go`, wired via `PreflightGateBuilder` on the `ops aws` registry entry) is the pre-send deny: a read-only invocation whose argv touches a sensitive pattern (a secrets / credentials / terraform-state / backup bucket glob, or an admin/root role ARN - the baked-in `defaultAWSSensitiveReadPatterns`, overridable wholesale via `aws.sensitive_read_patterns`) is refused before the aws CLI runs. Default-deny on the patterns, with two explicit-allow paths: a persistent `aws.allow_sensitive_reads` glob in `.coily/config.yaml`, or the per-invocation `COILY_AWS_ALLOW_SENSITIVE_READ` env var. The audit row lands for both outcomes so the trail survives the boundary: a denied read writes an `ops.aws.read.denied` (reject) row before the hard error, an explicitly-allowed sensitive read writes an `ops.aws.read.allowed` (accept) row alongside the passthrough's own `ops.aws` row. Scope is the read side only - destructive verbs are a different layer, already gated, and out of scope. Runtime verification: `TestSecurityClaim_AWSReadOnlyGateDeniesSensitiveReads` in `cmd/coily/security_claims_test.go` drives wiring and behavior, with the `TestAWSReadGate_*` / `TestIsAWSReadOnly` family in `cmd/coily/aws_read_gate_test.go` pinning the classifier and allow paths. Scoping rationale: coilyco-bridge/coily#54.
- **Inline program-authoring is denied: shell function definitions and `eval`.** The leading-token classifier (above) is blind to a command that *authors* an arbitrary program inline and runs it in the same Bash call, with no on-disk artifact to inspect - the laundering analogue of `chmod +x /tmp/foo && /tmp/foo` for code that never touches disk. `f() { ... }; f` defeats the model because the `;`/`&&` splitter fragments the function body and every resulting segment then leads with a benign token (the function name, `cd`); `eval "$code"` is the dynamic-code variant. cli-guard's hook adds a content-based check on the *raw* command, before the segment split, in `renderHookHeader` - so it covers both the per-repo hook and the unconditional user-level hook. It denies inline function definitions (`name() {`, `function name {`) and `eval` in command position, while `eval` as a bare argument (`grep eval file`, `nix eval`) passes. The deny message routes the agent to the artifact-producing path: write the logic to a file in the repo and execute it directly (`./script.sh`), which the scan can inspect before it runs. Runtime verification: `TestSecurityClaim_InlineAuthoringBlocked` in `cmd/coily/security_claims_test.go` drives the rendered user hook (funcdef + eval blocked; loop + eval-as-argument allowed). cli-guard#51.
- **Repo verbs may opt out of the shell-metacharacter validator per verb.** A `.coily/coily.yaml` command can declare `allow_metacharacters: true`, which skips `policy.ValidateArg` on the YAML argv tokens at load time and sets `verb.Spec.SkipPolicy` so user-supplied extras also bypass the check at invocation. The opt-in is declarative (lives in the committed yaml, visible in PR diffs), bounded per verb (no global flag), and the audit row stamps `policy_skipped: true` so forensics can see that the row ran under the relaxed policy. The validator stays correct by default for built-in verbs whose argv can reach a remote shell. User exec verbs invoke a binary directly via execve, so the metacharacter check was overcautious for that surface unless the wrapped binary itself shells out (in which case the repo author leaves the opt-in off). Runtime verification: `TestBuildChildRepoCommand_AllowMetacharactersStampsAudit` in `cmd/coily/ops_repo_test.go` drives both the bypass and the audit-row stamp. Scoping rationale: [coilysiren/coily#283](https://github.com/coilysiren/coily/issues/283) / [coilysiren/cli-guard#81](https://github.com/coilysiren/cli-guard/issues/81).
- **The built-in `ops forgejo` create/edit verbs apply the same opt-out via `verb.Spec.SkipPolicy = true`.** Their free-text args (issue/PR/milestone titles, label/release names) are `json.Marshal`'d into an HTTP request body by `forgejoAPIDo` (`net/http`, no subprocess) and never reach a shell or even execve, so the shell-metacharacter gate's threat model does not apply. Skipping it lets an idiomatic title like `feat(dispatch): x` through verbatim instead of bouncing on `(` or being lossily rewritten; each verb stamps `policy_skipped: true` via the shared `stampPolicySkipped` OnComplete so the relaxed policy is visible in forensics. This is not a boundary relaxation: removing the skip would only re-tighten the gate (titles would bounce), never widen the execution surface. The earlier `cosmeticSanitize` rewrite (coilysiren/coily#129) is retired - it mangled valid titles to appease a gate that had no reach over an HTTP body.
- **`.coily/coily.yaml` repo verbs require the declaring file committed and the branch synced.** A repo verb refuses when the declaring `.coily/coily.yaml` is among the dirty paths, when HEAD is detached, when the branch has no upstream, or when the branch is behind upstream (ahead is allowed). Without this gate, a repo verb's argv could be whatever a local edit said at that moment, and the audit row would not be reconstructable from `git log` alone. Working-tree dirt outside `coily.yaml` does not refuse - the committed `coily.yaml` plus HEAD still reconstruct the verb invocation - but the porcelain status is still captured on the audit row for forensics. The `--audit-override-dirty` flag bypasses the gate for genuine emergencies and tags the audit row with `audit_override: true`. Built-in verbs are unaffected: their behavior is baked into the homebrew-released binary and reproducible from the version field in the audit row. Runtime verification: `TestSecurityClaim_repo_verbs_require_clean_tree` in `cmd/coily/security_claims_test.go` drives the gate end-to-end. Scoping rationale: coilysiren/coily#211.
- **Every invocation is logged, and forensic provenance is best-effort by design.** There is one append-only row per invocation, routed to `~/.coily/audit/<owner>-<repo>.jsonl` by the cwd's git origin remote (or `_unrooted.jsonl` outside any repo). Each row carries a `repo_root` field stamped from `git rev-parse --show-toplevel` of cwd at invocation - empty when cwd is outside a repo, never a refusal. coily no longer binds an invocation to a repo or requires a `--commit-scope` flag: that coupling was removed (closes the commit-scope rip-out) because coily's role is host-wide and most invocations have nothing to do with the cwd's repo identity. `coily git audit-show --scope <repo> --since <ts>` replays the window by filtering on `repo_root`.
...
**Deny (privileged-op binaries, wholesale)**: `aws`, `kubectl`, `gh`, `docker`, `tailscale`, `helm`, `terraform`, `gcloud`, `tflint`, `tfsec`, plus broad `ssh` / `scp` / `rsync`. The wrapped binaries route through `coily ops <bin>` (for aws / kubectl / gh) or the matching coily verb; `ssh` / `scp` / `rsync` have no coily wrapper and are simply denied. Plus the execution-family blocks (scripting interpreters, shells, Windows scripting hosts, build runners, package managers, the non-Bash `PowerShell` tool).
