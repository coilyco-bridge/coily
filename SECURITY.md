...
- **No `coily shell` / `coily run` escape hatch**, ever. The moment one exists, the whole boundary collapses. Same rule applies to remote shells: no `coily ssh exec`, no `coily ops kubectl exec` pass-through. Where free-form remote shell is genuinely needed, the answer is to drop out to raw `ssh` and let the lockdown deny rule force an explicit override, not to wrap a free-form exec inside a coily verb. Named verbs (`coily ssh systemctl status <unit>`, `coily ssh deploy <name>`) cover the legitimate cases without restoring the escape.
- **`.coily/coily.yaml` repo verbs require a clean+synced tree.** A repo verb refuses to run unless the working tree has no uncommitted or untracked files, HEAD is on a branch tracking a remote, and the branch is not behind upstream (ahead is allowed). Without this gate, a repo verb's behavior is whatever the script declared in `coily.yaml` did at that moment, and a local edit becomes an off-host shadow that no auditor can reconstruct from `git log` alone. The `--audit-override-dirty` flag bypasses the gate for genuine emergencies but tags the audit row with `audit_override: true` and captures the porcelain status snapshot, so the run is still reconstructable. Built-in verbs are unaffected: their behavior is baked into the homebrew-released binary and reproducible from the version trailer in the audit row. Runtime verification: `TestSecurityClaim_repo_verbs_require_clean_tree` in `cmd/coily/security_claims_test.go` drives the gate end-to-end.
