...
promoted_to:
  - issue: https://github.com/coilysiren/agentic-os-kai/issues/254
---
...
No coily audit row exists for this denial because the rejection happened upstream of the gate. The agent followed `coily-ai/.claude/skills/daily-backlog/SKILL.md` verbatim, which under `## Procedure` says:
    ...
    python3 ~/projects/coilysiren/agentic-os-kai/.claude/skills/daily-backlog/script.py
...
The skill predates the `coily exec daily-backlog` verb. When the verb landed (recently, as part of the dailies-migration thread visible in agentic-os-kai recent commits: `26ebdd4 add daily-<name>-auth coily exec verbs`, `7d625e9 add remaining daily-* routines as coily exec entries`, etc.), the skill body was not updated to lead with the wrapper. The bare-python invocation stayed at the top of `## Procedure` as a documented contract.
