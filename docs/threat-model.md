...
- **Sub-tool binaries (`aws`, `kubectl`, `gh`) are embedded in the coily binary** via `//go:embed`, extracted to a per-user cache dir with checksum verification on each invocation, and exec'd from there. This prevents an agent from swapping out `/usr/local/bin/aws` to intercept coily's shell-outs. Tools with simple APIs (ssh/scp, tailscale) use Go SDKs instead, avoiding any subprocess entirely. No CLI is embedded, no argv ever reaches an OS exec layer for those verbs. Status: ssh is wired through `pkg/ssh` (golang.org/x/crypto/ssh, host keys verified against `~/.ssh/known_hosts`). scp is stubbed in the same package and lights up when a verb needs it. Tailscale SDK (`tailscale.com/client/tailscale`) is not pulled in yet because no verb consumes it. TODO: wire it when an eco verb (or anything else) needs to query tailnet state.
