...
**What it is**: pkg/auth issues tokens signed with an HMAC-SHA256 key stored at `~/.coily/token-issuer.key`. Created on first use with 0600 perms and 32 random bytes.
