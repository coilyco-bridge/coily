...
**Audit row binding**: every audit row carries a `commit_scope` field set from the `--commit-scope` global flag (or `$COILY_COMMIT_SCOPE`). Default is `auto`, which resolves to `git rev-parse --show-toplevel` of cwd at invocation time. If cwd is not inside a git repo, `auto` is an error - operators must pass `--commit-scope=<path>` explicitly. There is no user-facing opt-out: `-`, `none`, and `off` are all rejected with `ErrOptOutRejected`. The provenance contract is that every audit row binds to a real repo, period. Verbs that genuinely have no repo to bind to (rare) set `verb.Spec.SkipScope = true` at the definition site so the choice is auditable from the source, not papered over by a flag at the call site.
