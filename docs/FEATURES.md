...
**REST API wrappers**: `coily ops {modio,discord,sentry,trello,forgejo}`. The OpenAPI-generated surfaces (trello, sentry, discord, plus `coily pkg glama`) come from [`scripts/openapi-to-coily.py`](../scripts/openapi-to-coily.py); refresh them against upstream specs with [`scripts/regen-rest-wrappers.sh`](../scripts/regen-rest-wrappers.sh). Every wrapper carries a `--query`/`--output` projection rail ([coily#46](https://forgejo.coilysiren.me/coilyco-bridge/coily/issues/46), folded into the generator in [coily#182](https://forgejo.coilysiren.me/coilyco-bridge/coily/issues/182)): `--query` is a JMESPath projection over the JSON response and `--output` picks `json|yaml|text`, mirroring the AWS CLI. Both inherit to every leaf as persistent root flags; with neither set the response streams byte-for-byte as before. Search ops that already own an API `--query` expose the projection as `--jq` instead, mirroring the `coily ops gh` (gh-rest) `--jq` rail.
