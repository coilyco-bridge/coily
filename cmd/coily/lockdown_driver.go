...
// `coily lockdown`. As of coilysiren/coily#185, coily no longer ships
// its own 159-line POSIX shell deny/recovery script. Instead, the hook
// shims into `agent-guard hook pre-tool-use`, which carries:
//
//   - The required binary-path check for known guard binaries
//     (agent-guard#14), formerly coily's `<binary>-binary-gate.sh`.
//   - The routing-hint table for wrapped binaries (agent-guard#10),
//     formerly the giant `case` statement in `lockdown-deny.sh`.
//
// One source of truth, one binary on PATH, one set of tests.
